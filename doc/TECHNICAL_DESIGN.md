...
└── github.token            # GitHub token（可选，位于项目根目录）
```
...
#### 工作区识别
- 优先通过 `github.token` 和默认仓库目录结构识别 Plaud-App 工作区根目录
- 支持从工作区根目录、主仓库目录、子仓库目录执行 `mt`
- 不再依赖 `.mt-config.yaml`
...
1. **工作区识别**: 启动时自动识别 Plaud-App 工作区根目录
2. **路径验证**: 检查仓库路径是否存在
...
- ✅ **零配置运行**: 固定 7 个仓库，无需生成 `.mt-config.yaml`
- ✅ **命令验证**: 不支持的命令显示错误提示
...
MT 工具通过统一的命令行接口，简化了多仓库 Git 操作流程。采用 Shell 脚本实现，无需额外依赖，易于安装和维护。当前版本基于固定 7 仓库和自动工作区识别，默认零配置可用。
